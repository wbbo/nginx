...
ngx_int_t
ngx_open_one_listening_socket(ngx_listening_t *ls)
{
    int               reuseaddr;
    ngx_socket_t      s;
    ngx_err_t         err;
    ...
    if (ls->add_reuseport) {
        ...
        if (setsockopt(ls->fd, SOL_SOCKET, SO_REUSEPORT_LB,
...
void
ngx_close_one_listening_socket(ngx_listening_t *ls)
{
    ngx_connection_t *c;
